...
function customResolve(files: Files): PluginObj {
  return {
    visitor: {
      ImportDeclaration(path) {
        ...
        if (modulePath.startsWith(".")) {
          ...
          if (moduleFile.name.endsWith(".css")) {
            path.node.source.value = cssToJs(moduleFile);
          } else if (moduleFile.name.endsWith(".json")) {
            path.node.source.value = jsonToJs(moduleFile);
          } else {
